...
\begin{lstlisting}[language=Ruby,caption={\cite[p. 21]{Sto2013}}]
  100.times do
    Thread.new { sleep }
  end
...
\subsubsubsection{Thread-Safety in Ruby}
...
Der Conditional Assignment Operator in Ruby (\emph{\|\|\=}) wird häufig verwendet um einer Variable einen Standardwert zu zuweisen. Dabei ist dieser Operator nicht Thread-Safe, da er keine Atomare Operation darstellt. Hinter diesem Operator steht eine Funktion welche wie folgt aussehen könnte \footnote{Die genaue Implementierung dieses Operators kann sich zwischen den einzelnen Ruby Implementierungen unterscheiden. Das Funktionsweise ist bei allen Implementierungen gleich}:

\begin{lstlisting}[language=Ruby,label={listing:conditional},caption={\cite[p. 24]{Sto2013}}]
	\# Dieser Ausdruck
	@results ||= "test" 
...
\subsubsubsection{The GIL in MRI}
...
\begin{lstlisting}[language=Ruby,label={listing:fibonacci}caption={\cite[p. 21]{Sto2013}}]
class Fibonacci
...
\begin{lstlisting}[language=Ruby,label={listing:race condition},caption={\cite[p. 21]{Sto2013}}]
